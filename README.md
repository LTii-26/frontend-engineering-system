...
| 包名                                         | 路径                     | 说明                                                |
| -------------------------------------------- | ------------------------ | --------------------------------------------------- |
| `@frontend-engineering-system/ui`            | `packages/ui`            | 基础组件：`Badge` / `Button` / `Card` / `PageTitle` |
| `@frontend-engineering-system/utils`         | `packages/utils`         | 通用工具，例如`readBoolean`                         |
| `@frontend-engineering-system/eslint-config` | `packages/eslint-config` | 导出`base` / `react` 配置                           |
| `@frontend-engineering-system/ts-config`     | `packages/ts-config`     | 导出`base` / `node` / `react-app` 配置              |

应用通过 `workspace:*` 引用内部包，例如：

```json
"@frontend-engineering-system/ui": "workspace:*",
"@frontend-engineering-system/utils": "workspace:*"
```
...
```bash
pnpm --filter web dev
pnpm --filter @frontend-engineering-system/ui lint
pnpm --filter @frontend-engineering-system/utils typecheck
```
...
- **ESLint**：应用与包复用 `@frontend-engineering-system/eslint-config`
- **TypeScript**：复用 `@frontend-engineering-system/ts-config`
- **Prettier**：根目录统一格式化
- **Husky**
